"""Data-type mapping and round-trip fidelity (AC-TYPE-01..08)."""
...
def test_bigint_boundaries_are_exact(make_pipeline, documents) -> None:
    """Covers: AC-TYPE-08 — ±(2^63-1) survive as exact int64, never via float."""
    hi, lo = 2**63 - 1, -(2**63 - 1)
...
def test_timestamp_and_date_are_iso_strings(make_pipeline, documents) -> None:
    """Covers: AC-TYPE-02 — losslessly, incl. microseconds and offset."""
    ...
    # Lossless: the stored ISO-8601 string parses back to the exact same instant,
    # microseconds and offset intact (not just a prefix match).
...
def test_json_column_pinned_representation(make_pipeline, documents) -> None:
    """Covers: AC-TYPE-07 — json columns load as one canonical JSON string."""
    doc = _load_one(
        make_pipeline,
        documents,
